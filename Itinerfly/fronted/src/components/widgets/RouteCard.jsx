...
function RouteMapSVG({ from, to }) {
  ...
  const p1 = toXY(from.lat, from.lng);
  const p2 = toXY(to.lat,   to.lng);
...
export default function RouteCard({ route }) {
  const distancia = route.distance || (route.distanceKm ? `${route.distanceKm.toLocaleString()} km` : "—");
  const duracion  = route.duration || (route.durationMin ? `${Math.floor(route.durationMin/60)}h ${route.durationMin%60}m` : "—");

  const airlineName = route.airlineName || route.airlineId || '';

  return (
    <div className="route-card" >
      <div className="route-card__map">
        <RouteMapSVG from={route.from} to={route.to}/>
      </div>
      <div className="route-card__info">
        <div className="route-card__name">
          {route.to.name?.length > 32 ? route.to.name.slice(0,32)+"…" : (route.to.name || route.to.iata)}
        </div>
        ...
        <div className="route-card__pills">
          ...
          <span className="pill">{distancia}</span>
          <span className="pill">{duracion}</span>
        </div>
        {airlineName && (
          <div className="route-card__airline">{airlineName}</div>
        )}
      </div>
    </div>
  );
